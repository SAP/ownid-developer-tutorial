...
In case you are using the container with Gigya out-of-the-box integration or OwnID hosted solution you don't have to implement anything written in this document. The document refer to customers who are integrating OwnID library with their back-end code.

The server SDK exposes `/ownid/*` routs.
Each route will be used by OwnId UI SDK or OwnId Web Application.

* POST: `/ownid` - provides authorization context for UI SDK
* POST: `/ownid/{context}/start` - start communication between OwnID Web Application and server
* GET: `/ownid/{context}/challenge` - provides configuration for OwnId Web Application
* POST: `/ownid/{context}/challenge` - transfer user profile data entered on OwnId Web Application page according to configuration
...
* POST: `/ownid/{context}/approval-status` - check approval status if PIN validation initiated
* POST: `/ownid/{context}/approve` - set approval resolution for current context
